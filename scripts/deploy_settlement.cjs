#!/usr/bin/env node
/* Deployment script for AIMSSettlement on Base (CommonJS version).
 *
 * Usage:
 *   npx hardhat run scripts/deploy_settlement.cjs --network hardhat
 *
...
async function main() {
  ...
  // ── 5. Mint some MockUSDC to the deployer for testing ───────────────
  if (!isMainnet) {
  ...
  // ── 6. Print env vars for gateway configuration ─────────────────────
  console.log("\n=== Gateway Environment Variables ===");
  console.log(`AIMS_RPC_URL=http://127.0.0.1:8545`);
  console.log(`AIMS_CONTRACT_ADDRESS=${settlementAddress}`);
  console.log(`AIMS_USDC_ADDRESS=${usdcAddress}`);
  console.log(`AIMS_GATEWAY_PRIVATE_KEY=<deployer private key>`);
  console.log(`AIMS_TREASURY_ADDRESS=${PLATFORM_OWNER}`);
  console.log("==================================\n");
}
