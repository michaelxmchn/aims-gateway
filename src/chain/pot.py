...
``keccak256(abi.encodePacked(taskId, workerAddress))``.  Workers present
this to the on-chain contract to claim their 80 % reward.
"""
...
class POTManager:
    ...
    def generate_pot(self, task_id: str, worker_address: str) -> ProofOfTask:
        """Create and persist a PoT for the given task + worker.

        The signature is over ``keccak256(task_id ++ worker_address)``
        using the gateway's ECDSA key.
        """
        message_hash = keccak(
            text=f"{task_id}{worker_address}"
        )
        ...
        logger.debug(
            "PoT generated: task=%s worker=%s sig=%s…",
            task_id, worker_address, signature[:16],
        )
        return pot
    ...
    def verify_pot(
        ...
        The contract uses EIP-191 for on-chain verification; off-chain we
        recover the signer via ``eth_account``.
        """
        try:
            message_hash = keccak(
                text=f"{pot.task_id}{pot.worker_address}"
            )
            recovered = Account._recover_hash(message_hash, signature=pot.signature)
