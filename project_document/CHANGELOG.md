...
- feat(adapter-multisource): `scripts/bounty_adapter.py` 升级为 **多源聚合网关** — `MultiSourceAggregator` 协调 `BountycasterClient`(Source A) + `GitHubBountyClient`(Source B)；`DedupEngine` 按 `github_url` 去重；`MIN_BOUNTY_USD=10` 开枪门槛
- feat(github-bounties): `GitHubBountyClient` — 使用 GitHub Issues Search API 搜索含 bounty/reward/$/USDC 金额的开放 Issue，正则提取赏金金额，3 页 90 条上限，支持 `GITHUB_TOKEN` 环境变量提额至 5000 req/hr；`_extract_amount()` 多模式匹配（`$500`/`500 USDC`/`bounty: $75`）
- feat(sniper-mock-dual): `gitcoin_sniper.py` `_mock_bounties()` 从 3 条扩至 **6 条双源** mock（Bountycaster 3 + GitHub Bounties 3）：`gh-mock-001` $500 ERC-4626 合约、`gh-mock-002` $75 Express 限流中间件、`gh-mock-003` $200 DB 迁移 CLI；dry-run 46 条 Traces 41 ✅ 0 ⚠️ 0 🚨，5/6 匹配交付 $1,109 USDC，1 社交正确拒绝
### 新增
...
- feat(bountycaster-adapter): `scripts/bounty_adapter.py` — Bountycaster 生态适配器，将 Bountycaster API 转换为 Gitcoin 兼容格式的 HTTP 代理服务器（端口 9812）；轮询 `www.bountycaster.xyz/api/v1/bounties/open` + 独立 `/api/v1/bounty/{hash}` 详情渲染；支持 `--once`（单次抓取）、`--mock`（历史数据回放）、HTTP server 三种模式；内含 3 个真实历史 Bountycaster bounty 的 mock 数据
- feat(bountycaster-sniper): `scripts/gitcoin_sniper.py` mock 数据从虚构 Gitcoin bounties 迁移至真实历史 Bountycaster bounties：`bc-mock-001` 250 USDC NextJS Frame 开发、`bc-mock-002` 8,000 degen TypeScript 脚本、`bc-mock-003` 1 USDC 社交赏金；mock 评估器关键词扩展支持 `create/build/nextjs/react/typescript` 等构建类任务匹配；dry-run 19 条 Action Traces 17 ✅ 0 ⚠️ 0 🚨 — 2/3 代码 bounty 正确匹配交付，1/3 社交 bounty 正确拒绝
### 重构
### 重构
