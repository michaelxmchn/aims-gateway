...
- [x] **Bountycaster 生态迁移** — `scripts/bounty_adapter.py` Bountycaster API → Gitcoin-compatible HTTP 代理：轮询 `www.bountycaster.xyz/api/v1/bounties/open` + 独立 bounty 详情 `/api/v1/bounty/{hash}` 解析；`--once` 单次抓取；`--mock` 历史数据回放；`scripts/gitcoin_sniper.py` mock 数据从虚构 Gitcoin bounties 替换为真实历史 Bountycaster bounties（`bc-mock-001` 250 USDC Frame 开发、`bc-mock-002` 8,000 degen TypeScript 脚本、`bc-mock-003` 1 USDC 社交赏金拒绝）；mock 评估器关键词扩展支持 `create/build/nextjs/react/typescript` 匹配 Bountycaster 构建类型任务；dry-run 19 条 Action Traces 0 失败，2/3 匹配正确交付，1/3 社交正确拒绝
