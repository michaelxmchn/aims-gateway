...
- [x] **Algora.io Source C 并入多源聚合网关** — `scripts/bounty_adapter.py` `AlgoraClient` 使用 GitHub Issues Search `"/bounty" is:issue is:open` 跨引用发现 Algora 悬赏（因 Algora.io 为 Phoenix LiveView 无 JSON API）；`_parse_bounty_amount()` 正则匹配 `/bounty $X` 模式；3 个 Algora mock entries；`MultiSourceAggregator` 三源并网（Bountycaster + GitHub + Algora）
- [x] **Anti-bot Human Mimicry 反风控冷却** — `gitcoin_sniper.py` `AutoDeliverer.deliver()` 成功 PR 后 `random.uniform(180, 600)` 休眠模拟人类行为；日志 `[SECURITY] PR submitted successfully. Mimicking human behavior, cooling down for X seconds...`
- [x] **5-retry Circuit Break 熔断重试** — `gitcoin_sniper.py` `CodeExecutor.execute()` claude-code + 测试阶段包裹 `for attempt in range(1, 6)` 重试循环；全部 5 次耗尽后返回 FAILED；trace 日志 `Execution attempt N/5`
- [x] **Sniper 指向 Adapter 多源聚合网关** — `gitcoin_sniper.py` 新增 `AdapterPoller` 类（`new_bounties()`/`fetch()` 方法），`SniperPipeline` 在 LIVE 模式使用 `AdapterPoller` 替代 `BountycasterPoller`（DRY-RUN 仍保留 Bountycaster mock 数据）；新增 `ADAPTER_API=http://localhost:9812` 常量；新增 `grep` 预拉取；Docker 镜像 `node:20-slim` → `node:22-alpine`（本地已缓存）；`_run_in_docker()` 新增 Docker Hub 不可达优雅回退（exit 125 → None → 直连执行）；`_mock_evaluate()` 关键词扩展（solidity/contract/staking/reentrancy/overflow/security/vulnerability/injection）防误杀高价值 Solidity Bounty；移除非代码误报关键词 `"document"`；`AIMSEvaluator.evaluate()` 403 自动回退关键词启发式；`SNIPER_DOCKER=0` 环境变量实际控制执行路径
